...
import 'package:HFSPL/Review_Update_KYC_Photos/Rejected/group_card.dart';
import 'package:HFSPL/network/networkcalls.dart';
import 'package:HFSPL/network/responses/ReviewKyc/review_group.dart';
import 'package:HFSPL/utils/globals.dart';

class RejectedGroup extends StatefulWidget {
  const RejectedGroup({super.key});

  @override
  State<RejectedGroup> createState() => _RejectedGroupState();
}

class _RejectedGroupState extends State<RejectedGroup> {
  ...
  fetchPendingKycGroups() async {
    try {
      var response = await _client.getRejectedGroup(Global_uid);
  ...
  Widget build(BuildContext context) {
    ...
    if(pendingKycGroups.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Rejected Groups')),
        body: const Center(child: Text("No groups found")));
    }

    return Scaffold(
      appBar: AppBar(
        title: const Text('Rejected Groups'),
        actions: [
          Row(
            children: [
              AnimatedContainer(
